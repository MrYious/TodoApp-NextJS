...
export default function Task(props: { theme: string; toggleState: (id: string)=> void; handleDeleteTask: (id: string) => void; task: { id: string; state: string, text: string }}){
    ...
    return(
        <div key={props.task.id} className="flex py-4 px-5 items-center gap-5 border-b-[1px] border-gray-200 ">
            <span className="cursor-pointer select-none" onClick={()=>{props.toggleState(props.task.id)}}>
                {
                    props.task.state === "Active" ?
                        <div className={`${props.theme === 'light' ? 'border-gray-300 ' : 'border-gray-600'} hover:border-violet-600 rounded-full border-[1px] border-black w-6 h-6 shrink-0`}>
                        </div>
            ...
            >
                <div className={` ${props.task.state === 'Active' && props.theme === 'light' ? 'text-gray-500 font-medium' : props.theme === 'light' ? 'text-gray-300 line-through' : props.theme === 'dark' && props.task.state === 'Active' ? 'text-gray-300 font-medium' : ' text-gray-500 line-through' } w-full`} onClick={()=>{props.toggleState(props.task.id)}}>
                    {props.task.text}
                </div>
